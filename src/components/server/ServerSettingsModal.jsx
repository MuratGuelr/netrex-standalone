...
import { uploadServerIconToCloudinary, uploadBadgeIconToCloudinary, deleteImageFromCloudinary } from "@/src/utils/imageUpload";
...
export default function ServerSettingsModal({ isOpen, onClose, initialTab }) {
  const { 
    currentServer, updateServer, deleteServer, 
    roles, createRole, updateRole, deleteRole, 
    members, createInvite, activeInvites, fetchServerInvites, 
    kickMember, banMember, unbanMember, fetchBans,
    badges, createBadge, updateBadge, deleteBadge, assignBadgeToMember, removeBadgeFromMember
  } = useServerStore();
  ...
  const canViewBadges = isOwner || canManageRoles; // Same permission as roles
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case "badges":
        return canViewBadges ? <BadgesTab badges={badges} members={members} onCreate={createBadge} onUpdate={updateBadge} onDelete={deleteBadge} onAssign={assignBadgeToMember} onRemove={removeBadgeFromMember} showCreateModal={showCreateBadgeModal} setShowCreateModal={setShowCreateBadgeModal} serverId={currentServer.id} /> : null;
      case "members":
        ...
        {activeTab === "badges" && (
          <div className="absolute bottom-8 right-8 z-[100] animate-nds-fade-in">
            <Button 
              ...
              className="bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white shadow-[0_10px_30px_rgba(245,158,11,0.3)] hover:scale-105 active:scale-95 group transition-all h-12 px-6 rounded-2xl border-0"
            >
              <Plus size={20} className="mr-2 group-hover:rotate-90 transition-transform duration-300" />
              Rozet Oluştur
            </Button>
...
// Badges Tab - Sunucu Rozet Yönetimi
const BADGE_COLOR_PRESETS = [
  "#f59e0b", "#ef4444", "#22c55e", "#06b6d4", "#6366f1", "#8b5cf6", "#ec4899", "#f97316", "#14b8a6", "#3b82f6"
];

function BadgesTab({ badges, members, onCreate, onUpdate, onDelete, onAssign, onRemove, showCreateModal, setShowCreateModal, serverId }) {
